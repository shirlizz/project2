...
def create():
    ...
    if request.method == "POST":
        if len(newChannel) < 1 or newChannel is '':
            return render_template("error.html", message="channel can't be empty.")
...
def joined():
    ...
    emit('status', {
        ...
        'channel': room,
        'msg': 'Welcome ' + session.get('username') + ' to ' + session.get('current_channel') + '  Channel!'}, 
        room=room)
